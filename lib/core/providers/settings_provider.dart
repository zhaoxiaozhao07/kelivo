...
class SettingsProvider extends ChangeNotifier {
  ...
  // ===== Provider Groups =====
  List<ProviderGroup> _providerGroups = [];
  ...
  Future<void> _load() async {
    ...
    if (groupsStr != null && groupsStr.isNotEmpty) {
      try {
        final rawList = jsonDecode(groupsStr) as List<dynamic>;
        _providerGroups = rawList
            .map((e) => ProviderGroup.fromJson(e as Map<String, dynamic>))
            .toList();
      } catch (_) {}
    }
    // 数据迁移：如果没有分组数据，将所有现有供应商放入默认分组
    if (_providerGroups.isEmpty) {
      final allKeys = <String>[..._providersOrder];
      // 添加 providerConfigs 中存在但不在 order 中的 key
      for (final k in _providerConfigs.keys) {
        if (!allKeys.contains(k)) {
          allKeys.add(k);
        }
      }
      _providerGroups = [ProviderGroup.createDefault(providerKeys: allKeys)];
    }
    // load pinned models
  ...
  /// 保存分组数据到持久化存储
  Future<void> _saveProviderGroups() async {
    final prefs = await SharedPreferences.getInstance();
    final json = jsonEncode(_providerGroups.map((g) => g.toJson()).toList());
    await prefs.setString(_providerGroupsKey, json);
  }

  /// 设置全部分组
  Future<void> setProviderGroups(List<ProviderGroup> groups) async {
    _providerGroups = List.from(groups);
    notifyListeners();
    await _saveProviderGroups();
  }

  /// 添加新分组
  Future<void> addProviderGroup(String name) async {
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    final group = ProviderGroup(id: id, name: name);
    _providerGroups.add(group);
    notifyListeners();
    await _saveProviderGroups();
  }

  /// 删除分组（供应商移入默认分组）
  Future<void> removeProviderGroup(String groupId) async {
    // 不能删除默认分组
    if (groupId == ProviderGroup.defaultGroupId) return;

    final idx = _providerGroups.indexWhere((g) => g.id == groupId);
    if (idx < 0) return;

    final removed = _providerGroups.removeAt(idx);
    // 将被删除分组内的供应商移入默认分组
    final defaultIdx = _providerGroups.indexWhere(
      (g) => g.id == ProviderGroup.defaultGroupId,
    );
    if (defaultIdx >= 0 && removed.providerKeys.isNotEmpty) {
      final defaultGroup = _providerGroups[defaultIdx];
      _providerGroups[defaultIdx] = defaultGroup.copyWith(
        providerKeys: [...defaultGroup.providerKeys, ...removed.providerKeys],
      );
    }
    notifyListeners();
    await _saveProviderGroups();
  ...
  /// 更新分组
  Future<void> updateProviderGroup(ProviderGroup group) async {
    final idx = _providerGroups.indexWhere((g) => g.id == group.id);
    if (idx < 0) return;
    _providerGroups[idx] = group;
    notifyListeners();
    await _saveProviderGroups();
  }

  /// 切换分组启用状态（同时更新组内所有供应商）
  Future<void> toggleGroupEnabled(String groupId) async {
    final idx = _providerGroups.indexWhere((g) => g.id == groupId);
    if (idx < 0) return;

    final group = _providerGroups[idx];
    final newEnabled = !group.enabled;
    _providerGroups[idx] = group.copyWith(enabled: newEnabled);

    // 同步更新组内所有供应商的启用状态
    for (final key in group.providerKeys) {
      final cfg = _providerConfigs[key];
      if (cfg != null && cfg.enabled != newEnabled) {
        _providerConfigs[key] = cfg.copyWith(enabled: newEnabled);
      }
    }
  ...
  /// 切换分组折叠状态
  Future<void> toggleGroupCollapsed(String groupId) async {
    final idx = _providerGroups.indexWhere((g) => g.id == groupId);
    if (idx < 0) return;

    final group = _providerGroups[idx];
    _providerGroups[idx] = group.copyWith(collapsed: !group.collapsed);
    notifyListeners();
    await _saveProviderGroups();
  }

  /// 将供应商移动到指定分组
  Future<void> moveProviderToGroup(
    String providerKey,
    String? targetGroupId,
  ) async {
    // 从所有分组中移除该供应商
    for (int i = 0; i < _providerGroups.length; i++) {
      final g = _providerGroups[i];
      if (g.providerKeys.contains(providerKey)) {
        _providerGroups[i] = g.copyWith(
          providerKeys: g.providerKeys.where((k) => k != providerKey).toList(),
        );
      }
    }

    // 添加到目标分组（如果 targetGroupId 为 null，则添加到默认分组）
    final targetId = targetGroupId ?? ProviderGroup.defaultGroupId;
    final targetIdx = _providerGroups.indexWhere((g) => g.id == targetId);
    if (targetIdx >= 0) {
      final target = _providerGroups[targetIdx];
      _providerGroups[targetIdx] = target.copyWith(
        providerKeys: [...target.providerKeys, providerKey],
      );
    }
  ...
  /// 在分组内重新排序供应商
  Future<void> reorderProvidersInGroup(
    String groupId,
    int oldIndex,
    int newIndex,
  ) async {
    ...
    final group = _providerGroups[idx];
    final keys = List<String>.from(group.providerKeys);
    ...
    _providerGroups[idx] = group.copyWith(providerKeys: keys);
    notifyListeners();
    await _saveProviderGroups();
  }
  ...
  Future<void> reorderGroups(int oldIndex, int newIndex) async {
    ...
    final item = _providerGroups.removeAt(oldIndex);
    if (newIndex > oldIndex) newIndex--;
    _providerGroups.insert(newIndex, item);

    notifyListeners();
    await _saveProviderGroups();
  }

  /// 确保供应商在某个分组中（用于新增供应商时）
  Future<void> ensureProviderInGroup(
    String providerKey, {
    String? groupId,
  }) async {
    // 检查是否已在某个分组中
    ...
    // 添加到指定分组或默认分组
    await moveProviderToGroup(providerKey, groupId);
  }

  /// 从所有分组中移除供应商（用于删除供应商时）
  Future<void> removeProviderFromAllGroups(String providerKey) async {
    bool changed = false;
    for (int i = 0; i < _providerGroups.length; i++) {
      final g = _providerGroups[i];
      if (g.providerKeys.contains(providerKey)) {
...
class ProviderGroup {
  ...
  factory ProviderGroup.fromJson(Map<String, dynamic> json) => ProviderGroup(
    ...
    providerKeys:
        (json['providerKeys'] as List?)?.map((e) => e.toString()).toList() ??
        const [],
  );

  /// 创建默认分组
  static ProviderGroup createDefault({
    String name = '默认分组',
    List<String> providerKeys = const [],
  }) => ProviderGroup(
    id: defaultGroupId,
    name: name,
    enabled: true,
